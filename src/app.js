...
// Chinese retry overlay + capped-backoff auto-reload, and re-throws to abort the
// rest of module init (renderer is required everywhere below). See glResilience.js.
const { renderer, scene, camera, playerRig, setWorld } = guardGlContext(() => createScene(canvas));
...
renderer.setAnimationLoop(() => {
  ...
  // ONE render pass, identical for flat and XR. Skyboxes are ordinary layer-0
  // meshes (worldConvention.applySkyboxTweaks) depth-sorted with everything else,
  // so PC and VR render byte-for-byte the same. Camera near/far are owned solely
  // by scene.js (config NEAR/FAR); nothing here mutates them.
  renderer.render(scene, camera);
});

// --- Service worker + PWA update detection (canonical 4-path pattern) ---
// The whole app-side SW lifecycle lives in swUpdates.js (register timing, the
// 4 detection paths, skip-waiting reload, force-reset, version watermark). Wired
// at MODULE TOP-LEVEL — do NOT move this call into an event listener (pitfall #0
// in swUpdates.js: a type=module `load` may already have fired). app.js owns the
// DOM handles + status line; swUpdates owns the SW wiring behind this one call.
installSwUpdates({ updateToast, updateReload, forceUpdateButton, drawerVersion, setStatus });
