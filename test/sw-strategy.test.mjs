...
// 的回归守护 + .glb passthrough 红线守护（世界归 IDB sync，SW 不碰）。抄 WebPaint test/sw-strategy.test.mjs。
...
describe("service-worker · 策略路由 (prod cache-first / dev network-first)", () => {
  ...
  // 红线：.glb/.gltf 永不进 SW 缓存——世界新鲜度归 app IDB sync via If-None-Match（两个 scope 都放行）。
  it("红线：prod .glb passthrough（不 respondWith）", async () => {
    const sw = loadSW("/service-worker.js");
    sw.setFetch(async () => new MockResponse("GLB"));
    const r = await drive(sw.handlers, { url: `${ORIGIN}/worlds/home.glb`, mode: "cors" });
    eq(r, null, "prod 不许碰 .glb（留给 IDB sync）");
  });

  it("红线：dev .gltf passthrough（不 respondWith）", async () => {
    const sw = loadSW("/dev/service-worker.js");
    sw.setFetch(async () => new MockResponse("GLTF"));
    const r = await drive(sw.handlers, { url: `${ORIGIN}/dev/worlds/home.gltf`, mode: "cors" });
    eq(r, null, "dev 也不许碰 .gltf");
  });
});
